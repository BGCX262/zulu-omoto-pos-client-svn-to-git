...
namespace Zulu.BusinessService.Util
{
    public class ZuluHelper
    {
		...
		public static void CreateLog(string logMessage, string formName, LogSeverityTypeEnum logSeverityTypeEnum, string exceptionMessage)
		{
			Log log = new Log();

			int userID = 0;

			if(ZuluContext.Current.CurrentUser != null)
				userID = ZuluContext.Current.CurrentUser.UserID;

			log.FormName = formName;
			log.Exception = exceptionMessage;
			log.CreatedOn = DateTime.Now;
			log.UserID = userID;
			log.Severity = (int)logSeverityTypeEnum;
